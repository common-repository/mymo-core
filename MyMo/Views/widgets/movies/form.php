<div class="mymo-widget mymo-widget-movies">
    <?php
    ...
    <p>
        <?php echo __('Sub Genres', 'mymo') ?>
        <select name="<?php echo $this->get_field_name('sub_genres') ?>[]" class="w-100" multiple>
            <?php foreach ($genres as $genre) : ?>
                <option value="<?php echo $genre->term_id ?>" <?php if (in_array($genre->term_id, $sub_genres)) echo 'selected'; ?>><?php echo $genre->name ?></option>
            <?php endforeach; ?>
        </select>
    </p>
